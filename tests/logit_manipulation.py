...
new_token = ' mundo'
new_token_old_tokenization = tokenizer.encode(new_token)

# MARK: First Test
# Can I force model.generate('O meu') generate " mundo"?
phrase = 'O meu'

tokenizer.add_tokens(new_token)
model.resize_token_embeddings(len(tokenizer))

# Set the embedding of the new token to the average of the old tokenization
embed = model.get_input_embeddings()
new_token_id = tokenizer.convert_tokens_to_ids(new_token)

with torch.no_grad():
    old_embeds = torch.stack([embed.weight[t] for t in new_token_old_tokenization])
    _ = embed.weight[new_token_id].data.copy_(old_embeds.mean(dim=0).to(DEVICE))



generation = utils.generate(model, tokenizer, phrase, max_new_tokens=1, device=DEVICE, output_hidden_states=True)

logits = generation.logits[0][0]   # The first [0] is to obtain the logits for the "0th" generated token (we only have max_new_tokens=1 so there is only 1 elem)
embed_out = generation.hidden_states[0][-1][0, -1]

logit_gradient = logits.max() - logits[new_token_id]
learning_rate = 2e-4

# Update the embed of the new token with the embed_new_token = embed_new_token + logit_gradient * embed_out
with torch.no_grad():
    embed_in = embed.weight[new_token_id]
    _ = embed.weight[new_token_id].data.copy_((embed_in + logit_gradient * embed_out * learning_rate).to(DEVICE))

# Validate new logits
generation = utils.generate(model, tokenizer, phrase, max_new_tokens=1, device=DEVICE, output_hidden_states=True)
print(generation.logits)

# It does seem like the logits of the new token is much higher, but let's check if we had the token in the phrase
generation = utils.generate(model, tokenizer, phrase + new_token, max_new_tokens=1, device=DEVICE, output_hidden_states=True)
print(generation.logits[0][0][:-1].max())
# So the logits are still too high for this token. This means the model will always try and generate this token for all the phrases




# MARK: Test 2 
# Testing the same learning rate but with the starting embed of the first token (so, " mundo" is " mund", "o"... So I will initialize the new_token_id embedding with the same as " mund" and go from there)
model, tokenizer = loader.load_model_and_tokenizer(model_name=model_name, device=DEVICE, tokenizer_kwargs={'padding_side': 'left'})

embed = model.get_input_embeddings()

# Initialize the embedding of the new_token with the embedding of the first token of "new_token"
tokenizer.add_tokens(new_token)
model.resize_token_embeddings(len(tokenizer))
with torch.no_grad():
    _ = embed.weight[new_token_id].data.copy_(embed.weight[new_token_old_tokenization[0]].clone())
...
# It does seem like the logits of the new token is much higher, but let's check if we had the token in the phrase
print(tokenizer.decode(utils.generate(model, tokenizer, phrase + new_token, max_new_tokens=10, device=DEVICE, return_dict_in_generate=False)[0]))
print(generation.logits[0])

# Now, this does work, as the next tokens generated are not always the same but, since we started with the token " mund" the most likely token afterwards is almost always "o"...
# We need a mixture of the FIRST and SECOND test, so maybe the weighted average initialization is a good start
...
for new_token in tqdm.tqdm(new_tokens, desc='Updating the embeddings for the new tokens'):
    ...
    # for phrase in tqdm.tqdm(phrases_to_generate_new_token, desc=f'  Generating tokens for new_token=`{new_token}` ', leave=False):

    #     generation = utils.generate(hacked_model, tokenizer, phrase, max_new_tokens=1, device=DEVICE, output_hidden_states=True)
